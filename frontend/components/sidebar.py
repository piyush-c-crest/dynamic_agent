...
from api.api_client import OrchestratorClient
...
def render_sidebar() -> Tuple[OrchestratorClient, Optional[Dict[str, Any]]]:
    with st.sidebar:
        ...
        with nav2:
            st.page_link("pages/History.py", label="History", icon="🗂️", use_container_width=True)
        ...
        if not online:
            st.caption("Set `ORCHESTRATOR_MODE=server` in `backend/.env`, then run `python main.py` from the backend folder.")
