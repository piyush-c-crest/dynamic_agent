...
async def run_static_prompt():
    ...
    if paths:
        print(f"[*] Workflow  : backend/data/runs/{paths.relative_path}/workflow.json")
        print(f"[*] Outputs   : backend/data/runs/{paths.relative_path}/outputs/")
        print(f"[*] Run log   : backend/data/logs/{paths.relative_path}/run.log")
...
def start_server():
    ...
    uvicorn.run("api.api:app", host=Config.HOST, port=Config.PORT, reload=True)
